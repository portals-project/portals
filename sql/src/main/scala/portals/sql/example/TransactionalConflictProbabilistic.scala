  ...
  val app = PortalsApp("app") {
    val generator1 = Generators.fromIteratorOfIterators[String](
      List(
        ...
        List(
          "SELECT '1', * FROM Book WHERE id IN (1, 2)",
        ).iterator
      ).iterator
    )
...
//          "INSERT INTO Book (id, title, \"year\", author) VALUES (2, 'The Hunchback of Notre-Dame', 1829, 0)",
          "INSERT INTO Book (id, title, \"year\", author) VALUES (2, 'The Hunchback of Notre-Dame', 1829, 0), (3, 'The Last Day of a Condemned Man', 1829, 0)",
        ).iterator,
        List(
          "SELECT '2', * FROM Book WHERE id IN (1, 2)",
        ).iterator
      ).iterator
    )
...
//  val system = Systems.interpreter()
  val system = new RandomInterpreter(Some(1)) // I1 success, Q1 Rollback,
  system.launch(app)
