...
object TaskExtensions:
  ...
  extension (t: TaskBuilder) {
    ...
    /** Vsm behavior factory
      *
      * The inner behavior should return the next state, for this we recommend
      * the use of [VSMExtension.processor], note that the use of
      * `Tasks.processor` will not work, as it returns `Unit` and not the next
      * behavior. Warning: do not use vsm for the inner behavior, this will lead
      * to an infinite loop and crash.
      *
      * Example:
      *
      * {{{
  ...
  extension [T, U](task: GenericTask[T, U, Nothing, Nothing]) {
    ...
    /** Behavior factory for looping behaviors over atoms. This will execute the
      ...
      * @return
      *   a looping task executing first the previous `task` and then `_task`
  ...
  extension [T, U](task: GenericTask[T, U, Nothing, Nothing]) {
    ...
    /** Wrapping around the behavior of a task. The wrapped behavior is
      * accessible for use.
      *
      * Example use:
      * {{{
  ...
  object StashExtension:
    extension (t: TaskBuilder) {
      ...
      /** Behavior factory to build a task that can stash messages, and unstash
        ...
        *
        * @see
        *   `portals.examples.distributed.actor.ActorRuntime` for example of
        *   use.
        */
