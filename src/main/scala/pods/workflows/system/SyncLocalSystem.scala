...
class RuntimeBehavior[I, O](
  ...
  def iref(): IStreamRef[I] = new IStreamRef[I] {
    private[pods] def submit(event: I): Unit = ???
    private[pods] val opr: OpRef[I, _] = null
  ...
  def oref(): OStreamRef[O] = new OStreamRef[O] {
    private[pods] val opr: OpRef[_, O] = null
    private[pods] def subscribe(subscriber: IStreamRef[O]): Unit = ???
  }
