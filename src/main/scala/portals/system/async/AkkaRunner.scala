...
import portals.Atom
import portals.AtomicStreamRef
import portals.Generator
import portals.Key
import portals.Sequencer
import portals.Task
import portals.TaskCallback
import portals.TaskContext
import portals.TaskContextImpl
import portals.Tasks
import portals.WrappedEvent

object Events:
  sealed trait Command
  case class Subscribe(subscriber: ActorRef[Command], replyTo: ActorRef[Reply]) extends Command
  case class Event[T](sender: String, event: WrappedEvent[T]) extends Command

  sealed trait Reply
  case object SubscribeSuccessfull extends Reply

object AtomicConnectionsExecutor:
  import Events.*

  def connect(stream: ActorRef[Command], sequencer: ActorRef[Command])(using Timeout, Scheduler) =
    val fut = stream.ask(replyTo => Subscribe(sequencer, replyTo))
    Await.result(fut, Duration.Inf)

object AtomicStreamExecutor:
  import Events.*

  /** Atomic Stream Executor.
    */
  def apply[T](path: String, subscribers: Set[ActorRef[Command]] = Set.empty): Behavior[Command] =
    Behaviors.setup { ctx =>
      ...
      Behaviors.receiveMessage {
        ...
        case Event(_, event) =>
          subs.foreach { _ ! Event(path, event) }
          Behaviors.same
      }
    }
end AtomicStreamExecutor // object

object AtomicGeneratorExecutor:
  import Events.*
  ...
  /** Generator Executor.
    */
  def apply[T](path: String, generator: Generator[T], stream: ActorRef[Command]): Behavior[GeneratorCommand] =
    Behaviors.setup { ctx =>
...
object AtomicWorkflowExecutor:
  import Events.*

  // The source simply forwards events to the subscribers.
  // No alignment is needed as it listens only to a single stream.
  object Source:
    def apply(path: String, subscribers: Set[ActorRef[Command]] = Set.empty): Behavior[Command] =
      Behaviors.setup { ctx =>
        Behaviors.receiveMessage {
          case Subscribe(subscriber, replyTo) => ??? // not supported
