...
class DataParallelSystem(val numPartitions: Int) extends SystemContext:
  import Events.*
  ...
  def launchStream[T](partition: Int, stream: AtomicStream[T]): Unit =
  ...
  def launchSequencer[T](partition: Int, sequencer: AtomicSequencer[T]): Unit =
    ...
    partitions(partition).sequencers += sequencer.path -> aref

  def launchConnection[T](partition: Int, connection: AtomicConnection[T]): Unit =
    AtomicConnectionsExecutor.connect(
  ...
  def launchWorkflow[T, U](partition: Int, workflow: Workflow[T, U]): Unit =
    ...
    workflow.sources.foreach { (name, t) =>
      ...
      AtomicConnectionsExecutor.connect(partitions(partition).streams(workflow.consumes.path), aref)
...
object ShuffleSender:
  ...
  def apply(partition: Int, path: String, subscribers: PartitionMapImpl[ActorRef[Command]]): Behavior[Command] =
    Behaviors.setup { ctx =>
...
object ShuffleReceiver:
  ...
  def apply[T](
      ...
      path: String,
      senders: Set[String],
      subscribers: Set[ActorRef[Command]]
  ): Behavior[Command] =
    Behaviors.setup { ctx =>
